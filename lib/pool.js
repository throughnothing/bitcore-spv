...
Pool.prototype.connect = function() {
  ...
  this.chain.on('progress', function(p) { self.emit('chain-progress', p) });
...
Pool.prototype._setLoaderPeer = function(peer) {
  ...
  console.log('getting blocks to catch up...', this.chain.index.lastHash, this.chain.index.lastHeight);
...
Pool.prototype._handlePeerMerkleBlock = function(peer, message) {
  console.log('peer-block', message);
  this.emit('peer-merkleblock', peer, message);
}
...
Pool.prototype._handlePeerInv = function(peer, message) {
  ...
  console.log('Inv tx', txHashes.length, 'blocks', blockHashes.length);
