...
function Pool(options) {
  ...
  this.pool = null;
  this.bloom = null;
  this.bloom = BloomFilter.create(1, 0.0001, 0, 0);
  this.ping = {
...
Pool.prototype.connect = function() {
  ...
  this.pool.on('peermerkleblock', this._handlePeerMerkleBlock.bind(this));
...
Pool.prototype.watch = function(txIds) {
  ...
  if(this.peers.loader) {
    console.log('updateFilter on peer loader');
  ...
  for(var i = 0; i < this.peers.pending.length; i++) {
    console.log('updateFilter on pending peer');
...
console.log('updateFilter waiting on peer ready');
    peer.once('ready', function() { self._updateFilter(peer) });
    return;
  }
  if(!this.chain.index.lastHash) {
console.log('updateFilter waiting on lastHash');
    this.chain.once('load', function() { self._updateFilter(peer) });
    return;
  }
  console.log('updateFilter past waits');

  console.log('Sending FilterLoad');
  peer.sendMessage(new Messages.FilterLoad(this.bloom));
  if(!this.relay) {
    peer.sendMessage(new Messages.GetBlocks([this.chain.index.lastHash],0));
  }
}
...
Pool.prototype._handlePeerMerkleBlock = function(peer, block) {
  this.emit('peer-merkleblock', peer, block);
}
...
console.log('INV blocks', blockHashes.length, 'tx', txHashes.length);
